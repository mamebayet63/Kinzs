...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty(array_filter($errors))) {
        ...
        if (move_uploaded_file($photo_couverture["tmp_name"], $target_file)) {
            ...
            $stmt = $pdo->prepare("INSERT INTO residence (nom, adresse, wifi, position, description, nbr_chambre, couverture) VALUES (:nom, :adresse, :wifi, :position, :description, :chambre, :image)");
            $stmt->execute([
                ':nom' => $nom,
